...
router.post('/admin/backfill-list-members', requireAdmin, requireFirestore, async (_req, res) => {
  try {
    ...
    // Treat any per-batch error as an overall failure so the operator
    // doesn't read partial-success counters as a clean run. The runbook
    // says to treat ok:true + errors:[] as the only acceptable outcome.
