...
// Acquire/release a transactional migration lock so only one long-running
// admin job is in flight at a time AND so /admin/unfreeze cannot lift the
// freeze while a scan is still mutating Firestore. Stored as
// configs/featureFlags.migrationInProgress: { jobId, startedAt }.
//
// Acquire is transactional: refuses if already held by another job. Caller
// must always release in a finally block (or via the runWithMigrationLock
// helper) so a thrown error doesn't leak the lock.
...
// Refuses if a migration job is in flight — without this, an operator
// could lift the freeze while a backfill scan is mid-loop, racing with
// reopened client writes.
router.post('/admin/unfreeze-list-membership', requireAdmin, requireFirestore, async (_req, res) => {
  try {
    ...
    if (snap.exists && snap.data().migrationInProgress) {
      const existing = snap.data().migrationInProgress;
      return res.status(409).json({
        error: `Cannot unfreeze: migration job is still in progress (jobId=${existing.jobId}). ` +
          'Wait for it to complete, or clear configs/featureFlags.migrationInProgress in the Firebase Console if it stalled.',
        migrationInProgress: existing,
      });
    }
    await ref.set({ freezeListMembershipWrites: false }, { merge: true });
