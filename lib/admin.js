  ...
  for (const listDoc of listsSnap.docs) {
    try {
      ...
      for (let i = 0; i < memberDocs.length; i += 1) {
        ...
        const pinListIds = pinSnap.exists
          ? (Array.isArray(pinSnap.data()?.listIds) ? pinSnap.data().listIds : [])
          : [];
        const isOrphan = !pinListIds.includes(listDoc.id);
        if (isOrphan) {
          batch.delete(
            firestore.collection('lists').doc(listDoc.id).collection('members').doc(memberDoc.id),
          );
          scrubsInBatch += 1;
        }
      }
      if (scrubsInBatch > 0) {
        try {
          await batch.commit();
          stats.orphansDeleted += scrubsInBatch;
        } catch (err) {
