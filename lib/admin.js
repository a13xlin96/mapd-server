...
const { runBackfillMealTypes } = require('../enrich/backfillMealTypes');
...
const { runBackfillCollaboratorProfiles } = require('./backfillCollaboratorProfiles');
const { runBackfillHomeLocations } = require('./backfillHomeLocations');
...
async function runBackfill() {
  const stats = {
    ...
    invalidPinListIds: [],  // {pinId} — pin.listIds present but not an array
    errors: [],
  };
  ...
  pinsSnap.forEach((doc) => {
    const data = doc.data();
    // Schema-drift safety: if listIds is present but not an array (e.g.
    // legacy pin with "L1" instead of ["L1"]), don't silently treat it as
    // empty — that would erase the pin's memberships from the new source-
    // of-truth without any operator-visible failure. Surface as a hard fail
    // so the operator can fix the source data first.
    if (data.listIds !== undefined && !Array.isArray(data.listIds)) {
      stats.invalidPinListIds.push({ pinId: doc.id });
      return;
    }
  ...
  // Apply each chunk via a TRANSACTION that re-reads parent list docs AND
  // existing member docs immediately before writing. Without this, a list
  // deleted between the up-front pre-pass and the chunk commit would land
  // member writes under a deleted parent — Firestore allows subcollection
  // writes under missing parents, so those orphans would survive both
  // runScrubOrphanMembers (only iterates existing /lists docs) and
  // runReconcilePinCounts. The transaction's read-then-write semantics mean
  // a parent-deleted-mid-chunk scenario aborts cleanly with no orphan rows.
  //
  // Transaction budget per chunk: at most 100 unique-list reads + 100
  // member-doc reads + 100 member-doc writes = 300 ops. Comfortably under
  // Firestore's 500 cap.
  for (let i = 0; i < entries.length; i += 100) {
    const chunk = entries.slice(i, i + 100);
    const chunkUniqueListIds = Array.from(new Set(chunk.map((e) => e.listId)));
    try {
      const result = await firestore.runTransaction(async (txn) => {
        // Re-validate parent list existence inside the transaction.
        const listRefs = chunkUniqueListIds.map((lid) =>
          firestore.collection('lists').doc(lid),
        );
        const listSnaps = await Promise.all(listRefs.map((ref) => txn.get(ref)));
        const aliveListSet = new Set();
        for (let k = 0; k < chunkUniqueListIds.length; k += 1) {
          if (listSnaps[k].exists) aliveListSet.add(chunkUniqueListIds[k]);
        }

        // Read existing member docs for idempotency check.
        const memberRefs = chunk.map((e) =>
          firestore.collection('lists').doc(e.listId).collection('members').doc(e.pinId),
        );
        const memberSnaps = await Promise.all(memberRefs.map((ref) => txn.get(ref)));
        ...
        for (let j = 0; j < chunk.length; j += 1) {
          const e = chunk[j];
          if (!aliveListSet.has(e.listId)) {
            // Parent list was deleted between pre-pass and this transaction.
            newlyStale.push({ pinId: e.pinId, listId: e.listId });
            continue;
          }
          const ex = memberSnaps[j];
          ...
          txn.set(memberRefs[j], e, { merge: true });
          chunkWrites += 1;
        }
        return { chunkWrites, chunkUnchanged, newlyStale };
      });
      stats.membersWritten += result.chunkWrites;
      stats.membersUnchanged += result.chunkUnchanged;
      for (const s of result.newlyStale) stats.staleListRefs.push(s);
    } catch (err) {
      stats.errors.push({ at: `chunk starting index ${i}`, error: err.message });
    }
  }
...
router.post('/admin/backfill-list-members', requireAdmin, requireFirestore, requireFrozen, async (_req, res) => {
  try {
    ...
    if (
      ...
      || stats.invalidPinListIds.length > 0
    ) {
...
// One-shot Google Places re-enrichment to backfill servesBreakfast/Lunch/
// Dinner/Brunch and priceRange onto existing pins. Independent of the
// collaborative-lists migration — does NOT require requireFrozen, and is
// safe to run while clients are writing because the patch only adds new
// fields to a pin doc the owner already controls.
router.post('/admin/backfill-meal-types', requireAdmin, requireFirestore, async (req, res) => {
  const { batchSize, startAfterDocId, dryRun, categoryFilter } = req.body || {};
  try {
    const stats = await runBackfillMealTypes({
      firestore,
      admin,
      batchSize: typeof batchSize === 'number' ? batchSize : undefined,
      startAfterDocId: startAfterDocId || null,
      // Default to dryRun=true unless explicitly disabled, so an accidental
      // empty-body POST never writes to Firestore.
      dryRun: dryRun !== false,
      categoryFilter: categoryFilter || 'food',
    });
    res.json({ ok: true, stats });
  } catch (err) {
    console.error('backfill-meal-types failed:', err);
    res.status(500).json({ error: err.message });
  }
});

// One-shot backfill to convert List.collaboratorProfiles from the legacy
// array shape to a UID-keyed map shape. Required after the 2026-05-16
// collab-hardening rule change — the new rules diff-validate the map per
// uid, which is not expressible on arrays. Safe to re-run (idempotent);
// dry-run by default to prevent accidental writes from an empty POST.
router.post('/admin/backfill-collaborator-profiles', requireAdmin, requireFirestore, async (req, res) => {
  const { batchSize, startAfterDocId, dryRun } = req.body || {};
  try {
    const stats = await runBackfillCollaboratorProfiles({
      firestore,
      batchSize: typeof batchSize === 'number' ? batchSize : undefined,
      startAfterDocId: startAfterDocId || null,
      dryRun: dryRun !== false,
    ...
    console.error('backfill-collaborator-profiles failed:', err);
    res.status(500).json({ error: err.message });
  }
});

// One-shot backfill that re-derives `category` and `cuisine` from already-
// stored `types[]` + `primaryType`. No Places API re-fetch — the source
// fields landed in the v4 Atmosphere backfill. Idempotent; dry-run by
// default. Safe to run while clients write (only patches owner-controlled
// fields).
// One-shot Places API re-enrichment for legacy food pins that are stuck
// with types:[] + primaryType:null and therefore can't be classified by
// backfill-category-cuisine. COSTS REAL PLACES API SPEND
// (~$0.017/pin actually fetched). Skips pins that already have a type
// signal so re-runs are cheap. Dry-run by default.
router.post('/admin/backfill-place-types', requireAdmin, requireFirestore, async (req, res) => {
  const { batchSize, startAfterDocId, dryRun } = req.body || {};
  if (batchSize !== undefined && (!Number.isInteger(batchSize) || batchSize < 1 || batchSize > 200)) {
    return res.status(400).json({ error: 'batchSize must be an integer between 1 and 200' });
  }
  if (startAfterDocId !== undefined && startAfterDocId !== null) {
    if (typeof startAfterDocId !== 'string' || startAfterDocId.length === 0) {
      return res.status(400).json({ error: 'startAfterDocId must be a non-empty string when provided' });
    }
  }
  try {
    const stats = await runBackfillPlaceTypes({
      firestore,
      batchSize: typeof batchSize === 'number' ? batchSize : undefined,
      startAfterDocId: startAfterDocId === undefined ? null : startAfterDocId,
      dryRun: dryRun !== false,
    });
    res.json({ ok: true, stats });
  } catch (err) {
    console.error('backfill-place-types failed:', err);
    res.status(500).json({ error: err.message });
  }
});

router.post('/admin/backfill-category-cuisine', requireAdmin, requireFirestore, async (req, res) => {
  const { batchSize, startAfterDocId, dryRun, acceptAnyCategory } = req.body || {};
  // Reject obviously bad batchSize at the boundary with a 400, before
  // hitting the helper. The helper itself also validates; this just gives
  // a cleaner error code for client-side automation.
  if (batchSize !== undefined && (!Number.isInteger(batchSize) || batchSize < 1 || batchSize > 1000)) {
    return res.status(400).json({ error: 'batchSize must be an integer between 1 and 1000' });
  }
  // Cursor validation at the route boundary. Previous version used
  // `startAfterDocId || null`, which silently coerced '' to null and made
  // a malformed cursor accidentally restart from page 1 — defeating the
  // cursor guard. Pass-through instead and explicitly reject bad shapes.
  if (startAfterDocId !== undefined && startAfterDocId !== null) {
    if (typeof startAfterDocId !== 'string' || startAfterDocId.length === 0) {
      return res.status(400).json({ error: 'startAfterDocId must be a non-empty string when provided' });
    }
  }
  // Strict boolean check — coercing arbitrary truthy values would let
  // strings like "false" enable the final-sweep behavior by accident.
  if (acceptAnyCategory !== undefined && typeof acceptAnyCategory !== 'boolean') {
    return res.status(400).json({ error: 'acceptAnyCategory must be a boolean when provided' });
  }
  try {
    const stats = await runBackfillCategoryCuisine({
      ...
      startAfterDocId: startAfterDocId === undefined ? null : startAfterDocId,
      dryRun: dryRun !== false,
      acceptAnyCategory: acceptAnyCategory === true,
    });
    res.json({ ok: true, stats });
  } catch (err) {
    console.error('backfill-category-cuisine failed:', err);
    res.status(500).json({ error: err.message });
  }
});

// One-shot backfill that resolves legacy User.homeCity into the structured
// User.homeLocation (Phase 1 schema). Best-effort: high-confidence matches
// written automatically; ambiguous left null so the user gets the migration
// modal sequence per plan v5 Task H. Dry-run by default.
router.post('/admin/backfill-home-locations', requireAdmin, requireFirestore, async (req, res) => {
  const { batchSize, startAfterDocId, dryRun } = req.body || {};
  try {
    const stats = await runBackfillHomeLocations({
      firestore,
      searchGooglePlaces,
      batchSize: typeof batchSize === 'number' ? batchSize : undefined,
      startAfterDocId: startAfterDocId || null,
    ...
    console.error('backfill-home-locations failed:', err);
    res.status(500).json({ error: err.message });
  }
});
