...
async function fetchInstagramCarouselPost(url) {
  ...
  const html = await fetchSourceHtml(embedUrl, {'User-Agent':MOBILE_UA,Accept:'text/html,application/xhtml+xml'});
  ...
  // Extract all display_url image references. The captured string is a JSON
  // string literal, so URLs contain Unicode escapes like `%3D` for `%3D`
  // in base64 padding. JSON-unescape each URL — raw literals break Anthropic's
  // image fetcher even though IG's CDN is lenient about backslashes.
  // CDN serves the same image at multiple resolutions (distinct ?stp= query
  // params), so dedupe by the stable filename segment.
  const legacyUrls = [...peeled.matchAll(/"display_url":"((?:[^"\\]|\\.)*)"/g)]
    .map((m) => unescapeJsonString(m[1]))
    .filter((u) => u.startsWith('https://scontent'));
  const urlMatches = metadata.slides.length ? metadata.slides : legacyUrls;
  ...
  if (slideThumbnails.length === 0 && !metadata.caption && !metadata.page.description) {
