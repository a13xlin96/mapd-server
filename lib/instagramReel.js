...
const {createMediaDescriptor,attachMediaDescriptor} = require('./media/mediaSource');
const MOBILE_UA =
...
async function fetchInstagramReelPost(url) {
  ...
  const mediaDescriptor=createMediaDescriptor({url:canonicalUrl,renditions:metadata.mediaRenditions,durationMs:metadata.durationMs});
  if (!ogDescription && !ogTitle && mediaDescriptor?.availability !== 'available') {
