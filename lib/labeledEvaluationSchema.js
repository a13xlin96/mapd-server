...
function validateCorpus(corpus) {
  if (corpus?.schemaVersion === 2) return require('./labeledEvaluationSchemaV2').validateCorpusV2(corpus);
...
function verifyEvidenceDirectory(corpus, directory) {
  if (corpus?.schemaVersion === 2) return require('./labeledEvaluationSchemaV2').verifyAssets(corpus, directory);
...
function sealCorpus(corpus, sealedAt = new Date().toISOString()) {
  ...
  return {schemaVersion: corpus.schemaVersion, corpusId: corpus.corpusId, corpusSha256: checksum(corpus),
...
function validatePredictions(predictions, corpus, manifest) {
  ...
  if (corpus.schemaVersion === 2) return require('./labeledPredictionV2').validatePredictionsV2(predictions, corpus, manifest);
