...
function runYtDlp(url) {
  return new Promise((resolve, reject) => {
    ...
    const proc = spawn('yt-dlp', ['--dump-single-json', '--no-download', '--retries', '0', '--extractor-retries', '0', '--', url]);

    let stdout = '';
    let stderr = '';

    proc.stdout.on('data', (d) => { stdout += d.toString(); if(Buffer.byteLength(stdout)>4*1024*1024) {proc.kill();reject({message:'Extraction output too large',code:'INPUT_TOO_LARGE'});} });
    ...
    proc.on('close', (code) => {
      ...
      try {
        ...
        const hasSubtitles = Object.keys(json.subtitles || {}).length > 0 || Object.keys(json.automatic_captions || {}).length > 0;
        (hasSubtitles ? extractSubtitles(json.webpage_url || url) : Promise.resolve(null)).then((subtitles) => {
          resolve({
            title,
            description,
  ...
  return new Promise((resolve) => {
    ...
    proc.on('close', () => {
      ...
      try {
        ...
        if(transcript.length>16000) return resolve(null);
