...
const {mediaFromYtDlp,attachMediaDescriptor} = require('./media/mediaSource');
...
function runYtDlp(url, options = {}) {
  ...
  return new Promise((resolve, reject) => {
    ...
    if (options.signal?.aborted) return reject(new EngineError('attempt_stopped',{stage:'source'}));
    ...
    let stopped = false;
    const onAbort=()=>fail(new EngineError('attempt_stopped',{stage:'source'}));
    const fail = error => {
      if (stopped) return;
      stopped = true;
    ...
    proc.on('close', (code) => {
      ...
      try {
        ...
        const descriptor=mediaFromYtDlp(json,url);
        result.mediaDiscoveryAttempted=true;
        if (options.mediaOnly) return resolve(attachMediaDescriptor(result,descriptor));
        extractSubtitleTracks(json, { deadline, englishOnly: options.multilingual === false, provider: classifyContentProvider(url) || 'source' }).then(({ tracks, failures }) => resolve(attachMediaDescriptor({
          ...result, subtitles: tracks.map(track => track.text).join(' ') || null,
          subtitle_tracks: tracks, subtitle_failures: failures,
        },descriptor))).catch(() => resolve(attachMediaDescriptor({ ...result, subtitles: null, subtitle_tracks: [],
          subtitle_failures: [failureOf(new EngineError('dependency_error', { stage: 'subtitles', provider: classifyContentProvider(url) || 'source' }))] },descriptor)));
      } catch {
...
// Preserve native cue times alongside the legacy flattened string. Gaps remain
// gaps; merely receiving a subtitle file is not proof the whole video was read.
function parseSubtitleCues(body,format) {
  const clean=text=>decodeEntities(text.replace(/<[^>]*>/g,'')).normalize('NFC').trim();
  const time=value=>value.replace(',','.').split(':').reduce((sum,part)=>sum*60+Number(part),0)*1000;
  let cues=[];
  if(format==='json3') {
    const json=JSON.parse(body);
    cues=(Array.isArray(json.events)?json.events:[]).map(e=>({startMs:Number(e.tStartMs),
      endMs:Number(e.tStartMs)+Number(e.dDurationMs),text:clean((e.segs || []).map(s=>typeof s.utf8==='string'?s.utf8:'').join(''))}));
  } else {
    for(const block of body.replace(/\r\n?/g,'\n').split(/\n[ \t]*\n/)) {
      const lines=block.split('\n');if(/^(?:NOTE|STYLE|REGION)(?:\s|$)/.test(lines[0]))continue;
      const at=lines.findIndex(line=>/-->/.test(line));if(at<0)continue;
      const m=lines[at].match(/^\s*((?:\d{2,}:)?\d{2}:\d{2}[.,]\d{3})\s+-->\s+((?:\d{2,}:)?\d{2}:\d{2}[.,]\d{3})/);
      if(m)cues.push({startMs:time(m[1]),endMs:time(m[2]),text:clean(lines.slice(at+1).join(' '))});
    }
  }
  let chars=0;
  return cues.filter(c=>Number.isFinite(c.startMs) && Number.isFinite(c.endMs) && c.startMs>=0 && c.endMs>c.startMs
    && c.endMs<=24*3600000 && c.text && (chars+=c.text.length)<=MAX_SUBTITLE_CHARS).slice(0,500)
    .map(c=>({...c,timing:'native'}));
}
  ...
  for (const selected of selectSubtitleTracks(metadata, options)) {
    ...
    try {
      ...
      tracks.push({ language: selected.language, provenance: selected.provenance, text: bounded,
        segments:parseSubtitleCues(body,selected.format),
...
module.exports = { runYtDlp, extractSubtitles, parseSubtitleCues };
