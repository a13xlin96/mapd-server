...
async function sendPushForJob(jobId, userId, status, extra = {}) {
  ...
  try {
    ...
    switch (status) {
      ...
      case 'failed':
        type = 'pin_failed';
        title = "Couldn't save this link";
        body = extra.failure?.message || 'Tap to retry or add the place manually';
        break;
      default:
        return;
    }

    const latest = (await firestore.collection('enrichmentJobs').doc(jobId).get()).data();
    if(latest?.failure?.code==='partial_save') {type='pin_failed';title='Some places saved';body=latest.failure.message;}
    else if(status==='failed' && latest?.failure?.message) body=latest.failure.message;
