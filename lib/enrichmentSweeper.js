  ...
  try {
    ...
    const [byHeartbeat, byDeadline, byExecution] = await Promise.all([
      ...
      firestore
        ...
        .get(),
      firestore.collection('enrichmentJobs').where('status','==','processing').where('engineDeadline','<',admin.firestore.Timestamp.fromMillis(now)).get(),
    ]);
    ...
    for (const docSnap of docs.values()) {
      ...
      try {
        ...
        const marked = await firestore.runTransaction(async (txn) => {
          ...
          txn.set(docSnap.ref, {
            status: 'failed',
            failure:failureOf(new EngineError('dependency_timeout',{stage:'execution'})),
