...
const { releaseAdmission } = require('./enrichAdmission');
const queueMaintenance = require('./queueMaintenance').createQueueMaintenance({ db: firestore, admin, push: sendPushForJob });
...
async function sweep() {
  if (!firestore) return;
  // This continues even when workers have no available execution slots.
  await queueMaintenance.sweep().catch(() => console.warn('Pending queue maintenance failed'));
  try {
    const now = Date.now();
    ...
    const [byHeartbeat, byDeadline, byExecution] = await Promise.all([
      firestore
      ...
      firestore
        ...
        .limit(100)
        .get(),
      firestore.collection('enrichmentJobs').where('status','==','processing').where('engineDeadline','<',admin.firestore.Timestamp.fromMillis(now)).limit(100).get(),
    ]);

    const docs = new Map();
    for (const d of byHeartbeat.docs) docs.set(d.id, d);
    for (const d of byDeadline.docs) docs.set(d.id, d);
    for (const d of byExecution.docs) docs.set(d.id,d);
    ...
    for (const docSnap of docs.values()) {
      ...
      try {
        ...
        const marked = await firestore.runTransaction(async (txn) => {
          ...
          if(data.engineDeadline) {if(ms(data.engineDeadline)>now && ms(data.updatedAt)>now-HEARTBEAT_STALE_MS) return false;}
          else if(ms(data.createdAt)>now-ABSOLUTE_DEADLINE_MS && ms(data.updatedAt)>now-HEARTBEAT_STALE_MS) return false;
          const failure = failureOf(new EngineError('dependency_timeout',{stage:'execution'}));
          await recordTerminal(txn, firestore, docSnap.ref, { ...data, status: 'failed', failure },
            { now, reason: 'execution_expired', processingMissingReason: 'not_observed' });
          txn.set(docSnap.ref, {
        ...
        if (userId) {
          await releaseAdmission(firestore, userId, docSnap.id);
