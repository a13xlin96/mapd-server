...
const {getEngineFeatures,assertQueueFleet,assertMediaFleet}=require('./engineRuntimeConfig');
...
async function admitEnrichmentJob(db,{jobId,userId,url,captionText,adminBypass,retryOf,retryKind,clientCapabilities},{features}={}) {
  ...
  if ((retryKind != null && (!['places','analysis'].includes(retryKind) || !retryOf))
      || (clientCapabilities != null && (!Array.isArray(clientCapabilities) || clientCapabilities.length > 1
        || clientCapabilities.some(value=>value!=='mediaRecoveryV1')))) return {code:400,body:{error:'invalid_job'}};
  ...
  return db.runTransaction(async txn=>{
    ...
    const base = existing || {userId,url,captionText:captionText || '',...(retryOf?{retryOf}:{}),
      ...(retryKind?{retryKind}:{}),...(clientCapabilities?{clientCapabilities}:{}),createdAt:ts()};
    ...
    try {
      ...
      snapshot = controller.forJob(userId, existing?.engineFeatures, base.clientCapabilities);
    ...
    try { await assertQueueFleet(txn, db, controller.queuePolicy); await assertMediaFleet(txn,db,snapshot); }
    catch (error) {
