...
const STAGES = ['media', 'media_download', 'frame_selection', 'transcription', 'video_vision', 'media_fusion', 'queue', 'source', 'metadata', 'extraction', 'ai', 'vision', 'matching', 'details', 'save', 'projection', 'delivery', 'other'];
const OUTCOMES = ['success', 'failed', 'partial', 'duplicate', 'no_place', 'confirmation', 'blocked', 'rate_limited', 'timeout', 'cancelled', 'unknown'];
const PROVIDERS = ['openai', 'anthropic', 'google', 'instagram', 'tiktok', 'youtube', 'redis', 'firestore', 'extractor', 'other'];
const LEGACY_TOKEN_KEYS = ['input', 'output', 'cacheRead', 'cacheWrite'];
const TOKEN_KEYS = [...LEGACY_TOKEN_KEYS, 'textInput', 'audioInput'];
const rateTokens = rate => rate.billing === 'audio_tokens' ? ['textInput','audioInput','output'] : LEGACY_TOKEN_KEYS;
const EVIDENCE = ['title', 'caption', 'hashtags', 'mentions', 'subtitles', 'image', 'location', 'audio', 'video_frames'];
const OPS = ['projectionLagMs', 'projectionMismatches', 'failedDeliveries', 'lookupReads', 'providerRejections', 'cooldownMs', 'audioSubmittedSeconds', 'downloadedBytes', 'framesDecoded', 'framesSelected', 'framePixels', 'mediaCpuMs', 'mediaWallMs', 'mediaEscalations', 'mediaCacheHits', 'mediaCoalesced'];
const audioRate = require('./engineBudgetPolicy').DEFAULT_MEDIA_PRICES.rates.openai_mini_transcribe;
const DEFAULT_PRICES = Object.freeze({schemaVersion: 2, asOf: '2026-09-21', currency: 'USD', rates: {
  openai_mini_transcribe: {provider:'openai', billing:'audio_tokens',
    usdPerMillion:Object.fromEntries(Object.entries(audioRate.microdollarsPerMillion).map(([key,value]) => [key,value/1e6])),
    estimateUsdPerMinute:audioRate.estimateMicrodollarsPerMinute/1e6},
}});
...
function validatePrices(table) {
  ...
  if (![1,2].includes(table.schemaVersion) || table.currency !== 'USD' || !/^\d{4}-\d{2}-\d{2}$/.test(table.asOf) || new Date(table.asOf).toISOString().slice(0, 10) !== table.asOf) throw new Error('Invalid price table date/version/currency');
  ...
  for (const [key, rate] of Object.entries(table.rates)) {
    ...
    exact(rate, ['provider', 'billing', 'usdPerCall', 'usdPerMillion', ...(table.schemaVersion === 2 ? ['estimateUsdPerMinute'] : [])], 'rate');
    if (!PROVIDERS.includes(rate.provider) || !['calls', 'tokens', ...(table.schemaVersion === 2 ? ['audio_tokens'] : [])].includes(rate.billing)) throw new Error('Invalid rate provider/billing');
    if (rate.billing === 'calls') {
      if (!finite(rate.usdPerCall) || rate.usdPerMillion !== undefined) throw new Error('Invalid call price');
    } else {
      exact(rate.usdPerMillion, rateTokens(rate), 'token prices');
      if (rate.usdPerCall !== undefined || rateTokens(rate).some(k => rate.usdPerMillion[k] !== null && !finite(rate.usdPerMillion[k]))) throw new Error('All four token rates must be numbers or explicit null');
    }
    if (rate.estimateUsdPerMinute !== undefined && (rate.billing !== 'audio_tokens' || !finite(rate.estimateUsdPerMinute))) throw new Error('Invalid duration estimate');
  }
  return clone(table);
}
function estimateCall(call, table) {
  ...
  for (const key of rateTokens(rate)) {
    const n = call.tokens[key] ?? null, price = rate.usdPerMillion[key];
...
function createMetrics({platform, language, featureVersion, featureVersions = [], prices = DEFAULT_PRICES,
  ...
  const api = {
    ...
    providerCall({provider, rateKey, stage, outcome, tokens = {}, submittedAudioSeconds = null} = {}) {
      if (!accept()) return;
      ...
      data.providerCalls.push({provider: label(provider, PROVIDERS, 'other'), rateKey: Object.hasOwn(table.rates, rateKey) ? rateKey : 'unknown',
        ...
        submittedAudioSeconds: finite(submittedAudioSeconds) ? submittedAudioSeconds : null,
        // A planning estimate, deliberately excluded from estimateCall/cost totals.
        durationEstimateUsd: finite(submittedAudioSeconds) && finite(table.rates[rateKey]?.estimateUsdPerMinute) ? submittedAudioSeconds * table.rates[rateKey].estimateUsdPerMinute / 60 : null});
    },
...
function current() {const ctx = jobContext.current(); return ctx ? contexts.get(ctx) || ctx.sharedMetrics : undefined;}
...
function summarizeMetrics(reports) {
  ...
  for (const r of reports) {
    ...
    for (const call of r.providerCalls) {
      if (!PROVIDERS.includes(call.provider) || !call.tokens || (r.schemaVersion === 1 ? LEGACY_TOKEN_KEYS : TOKEN_KEYS).some(k => call.tokens[k] !== null && !count(call.tokens[k]))) throw new Error('Invalid provider usage');
    }
...
module.exports = {VERSION, STAGES, PROVIDERS, TOKEN_KEYS, OPS, PLATFORMS, LANGUAGES, DEFAULT_PRICES, validatePrices, createMetrics, start, current, persistPrivate,
