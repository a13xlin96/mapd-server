...
// Route parameters also contain a shortcode (and a boolean `caption`). They
// identify a page, not a post, and can appear before the actual media payload.
function hasPostData(value) {
  const typedMedia = ['GraphVideo','GraphImage','GraphSidecar','XDTGraphVideo','XDTGraphImage','XDTGraphSidecar'].includes(value.__typename)
    || [1,2,8].includes(value.media_type);
  const partialMetadata = [value.owner?.username,value.user?.username,value.location?.name]
    .some(field=>typeof field === 'string' && field.trim().length > 0)
    || Number.isFinite(value.video_duration) && value.video_duration > 0;
  return typeof value.caption?.text === 'string'
    || typeof value.video_url === 'string' || typeof value.display_url === 'string'
    || Array.isArray(value.video_versions) || Array.isArray(value.image_versions2?.candidates)
    || Array.isArray(value.edge_media_to_caption?.edges)
    || Array.isArray(value.edge_media_to_tagged_user?.edges)
    || Array.isArray(value.edge_sidecar_to_children?.edges)
    || Array.isArray(value.usertags?.in) || Array.isArray(value.coauthor_producers)
    || Array.isArray(value.carousel_media)
    || typedMedia && partialMetadata;
}
function parseInstagramPost(html, shortcode) {
  ...
  while (stack.length && visits++ < 30000) {
    ...
    if (typeof value === 'string' && /^[\s]*[\[{]/.test(value)) {
      ...
      seen.add(value);
      if ((value.shortcode === shortcode || value.code === shortcode) && hasPostData(value)) {post = value; break;}
