...
// Codex final-review finding: the alreadyResolved skip was treating ANY
// object-shaped homeLocation as resolved, but downstream consumers
// (getUserHomeLocation, client coerceHomeLocation) reject objects with
// missing/NaN/out-of-range coords. A partial/malformed doc would never
// be repaired, while distance calculations would silently produce null.
// Mirror the strict validation those readers use.
function isHomeLocationValid(home) {
...
async function runBackfillHomeLocations({
  ...
  while (true) {
    ...
    for (const doc of snap.docs) {
      ...
      // Skip if already has a VALID structured homeLocation (idempotent).
      // Malformed objects (missing/NaN/out-of-range coords) fall through to
      // the lookup path so we attempt repair instead of leaving the user
      // in a sticky partial-migration state.
      if (isHomeLocationValid(data.homeLocation)) {
      ...
      if (!dryRun) {
        ...
        try {
          await firestore.runTransaction(async (txn) => {
            ...
            if (isHomeLocationValid(freshData.homeLocation)) {
              // User (or another writer) populated a VALID value while we
              // were looking — respect their choice, don't clobber. A
              // malformed object is treated as "not yet resolved" and the
              // backfill goes ahead with the lookup result.
              return;
            }
