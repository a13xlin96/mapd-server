    ...
    try {
      const result = await firestore.runTransaction(async (txn) => {
        ...
        if (existingOverrides !== undefined) {
          ...
          // a null-clear). Build the skip set from bodyCheck.dotPaths,
          // each of which looks like "overrides.<fieldName>".
          const touchedFields = new Set();
          for (const dotPath of Object.keys(bodyCheck.dotPaths)) {
            const m = /^overrides\.(.+)$/.exec(dotPath);
            if (m) touchedFields.add(m[1]);
          }
        ...
        // Codex round-9 F44: foreign-pin overrides need an audit/event
        // trail so the pin's owner can see "Alex changed how your pin
        // shows in list X" in their feed, and so support/admins have a
        // durable record of cross-user changes for dispute / rollback.
        // Mirrors the list_member_removed_by_editor event from the
        // remove endpoint. Skip self-action (caller IS the pin owner).
        if (memberData.pinOwnerId !== callerUid) {
          const changedFields = Object.keys(bodyCheck.dotPaths)
            .map((p) => p.replace(/^overrides\./, ''));
          const eventRef = firestore.collection('events').doc();
          txn.set(eventRef, {
