...
//
// If the /users/{uid} doc doesn't exist at all, fail soft rather than
// blocking the join: firstName '' and photoURL null (no name/photo to show
// yet, but the membership itself still succeeds).
function buildJoinerProfile(uid, userSnap) {
  ...
  if (userSnap.exists) {
    ...
    firstName = rawFirstName || rawDisplayName.split(' ')[0] || 'User';
...
router.post('/lists/join', authenticateRequest, requireFirestore, async (req, res) => {
  ...
  try {
    const snap = await firestore.collection('lists')
    ...
    if (list.ownerId === uid) {
      return res.status(409).json({ error: 'own_list' });
    }
    if (list.deletePending === true) {
      return res.status(409).json({ error: 'list_deleting' });
    }
    if (Array.isArray(list.collaboratorIds) && list.collaboratorIds.includes(uid)) {
      return res.json({ alreadyMember: true, listId: listDoc.id, listName: list.name || '' });
    }
    ...
    return res.json({ joined: true, listId: listDoc.id, listName: list.name || '' });
  } catch (err) {
