...
router.get('/lists/:listId/visits', authenticateRequest, requireFirestore, async (req, res) => {
  ...
  try {
    ...
    //
    // FIELD MINIMIZATION: visit docs also carry `wouldGoBack`,
    // `visitNote` (a free-text personal note) and `verdictReason` — none
    // of which the visitedBy chips consume, and none of which should
    // cross into another collaborator's response. Only the fields the
    // client actually uses for this feature are returned below.
    const visits = [];
