  ...
  for (const stringField of ['placeName', 'formattedAddress']) {
    if (Object.prototype.hasOwnProperty.call(overrides, stringField)) {
      ...
      if (v === null) {
        ...
        // Reuse the truncate helper from the activity-event path —
        // unicode-safe code-point cap at OVERRIDE_FIELD_MAX.
        const truncated = truncate(v, OVERRIDE_FIELD_MAX);
        if (truncated.length === 0) {
          return {
            ok: false,
            status: 400,
            error: `${stringField} override cannot be empty — use null to clear`,
          };
        }
        dotPaths[`overrides.${stringField}`] = truncated;
      } else {
        return {
          ok: false,
          status: 400,
  ...
  async (req, res) => {
    ...
    try {
      const result = await firestore.runTransaction(async (txn) => {
        ...
        const [flagsSnap, listSnap, memberSnap] = await Promise.all([
          txn.get(flagsRef),
          txn.get(listRef),
          txn.get(memberRef),
        ]);
