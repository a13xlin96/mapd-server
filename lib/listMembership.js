  ...
  async (req, res) => {
    ...
    try {
      const result = await firestore.runTransaction(async (txn) => {
        ...
        // Trust-boundary check: the member doc may exist in a stale state
        // (e.g., previous backfill drift) where the pin doesn't actually
        // claim membership in this list. In that case we MUST NOT touch
        // the pin doc or send an activity notification — that would be a
        // spurious "your pin was removed" event to an uninvolved user
        // and a write against an unrelated pin's listIds. We DO still
        // delete the member doc and decrement pinCount, since cleaning
        // up the list-side drift is correct.
        const pinData = pinSnap.exists ? pinSnap.data() : null;
        const pinClaimsMembership =
          pinData && Array.isArray(pinData.listIds) && pinData.listIds.includes(listId);
