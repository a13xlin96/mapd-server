...
//   - Reject ASCII/C1 control chars except space and tab — newlines and
//     carriage returns inside a "place name" or "address" override are
//     a smell and would corrupt logs / single-line UI rendering.
//   - Reject angle brackets — closes the most common HTML/script injection
//     surface even though the React-Native Text component escapes content
//     by default; consumers outside RN (server logs, web admin views,
//     export paths) are not guaranteed to escape.
//   - Reject Unicode bidi-override control characters — used to spoof
//     filename/identifier rendering (Trojan-Source-style attacks).
//
...
// eslint-disable-next-line no-control-regex
const CONTROL_OR_BIDI_RE = /[\x00-\x08\x0a-\x1f\x7f-\x9f‎‏‪-‮⁦-⁩]/;
function sanitizeDisplayString(value) {
  ...
  if (CONTROL_OR_BIDI_RE.test(trimmed)) {
    return { ok: false, error: 'contains forbidden control or bidi characters' };
  }
