...
function validateStoredOverrides(stored) {
    ...
    try {
      const result = await firestore.runTransaction(async (txn) => {
        ...
        if (existingOverrides !== undefined && existingOverrides !== null) {
          ...
          // Codex round-6 F38: also validate the children. If a prior
          // buggy/manual write poisoned overrides.placeName with HTML or
          // a control char, an unrelated patch like
          // {overrides:{category:'food'}} would leave the bad value
          // untouched and still readable downstream. Fail closed instead.
          const childError = validateStoredOverrides(existingOverrides);
          if (childError) {
            return {
              ok: false,
              status: 409,
