  ...
  async (req, res) => {
    ...
    try {
      const result = await firestore.runTransaction(async (txn) => {
        ...
        // Codex round-2 F31: validate the member doc's own invariants
        // before mutating. runBackfill() repairs deterministic member
        // fields with merge:true semantics that PRESERVE extra fields
        // like `overrides`. If we apply an override to a corrupted member
        // doc (wrong pinId or pinOwnerId), the bad override value would
        // survive the later authoritative repair and end up attached to
        // the corrected membership. Fail closed so reconcile can clear
        ...
        if (memberData.pinOwnerId !== pinData.userId) {
          return {
            ok: false,
            status: 409,
